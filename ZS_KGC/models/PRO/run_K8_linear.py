...
class Runner:
    def __init__(self, args):
        ...
        node_feats5, node_feats6, node_feats7, node_feats8 = self.extract_disentangled_features(args)
        ...
        if args.intra_atten:
            ...
            self.fn = FN(self.labels.shape[1] * 8, self.labels.shape[1]).cuda()
    ...
    def extract_disentangled_features(self, args):
        ...
        if args.DATASET == 'NELL':
            embed_file = os.path.join(embed_path, 'Onto_NELL/DOZSL_GNN_K8_D200_NELL',
                                      '3000_2975_ent_embeddings.npy')
            entity_file = os.path.join(embed_path, 'Onto_NELL/ent2id.txt')
        if args.DATASET == 'Wiki':
            embed_file = os.path.join(embed_path, 'Onto_Wiki/DOZSL_GNN_K8_D200_Wiki',
                                      '2400_2366_ent_embeddings.npy')
            entity_file = os.path.join(embed_path, 'Onto_Wiki/ent2id.txt')

        ent2id = json.load(open(entity_file))

        embeds = np.load(embed_file)
        feat1_list, feat2_list, feat3_list, feat4_list, feat5_list, feat6_list, feat7_list, feat8_list = [], [], [], [],[], [], [], []
        ...
        return np.array(feat1_list), np.array(feat2_list), np.array(feat3_list), np.array(feat4_list), \
               np.array(feat5_list), np.array(feat6_list), np.array(feat7_list), np.array(feat8_list)
    ...
    def train(self, epoch):
        ...
        output_vectors = self.fn(torch.cat((output_vectors1, output_vectors2, output_vectors3, output_vectors4, output_vectors5,
                                            output_vectors6, output_vectors7, output_vectors8), 1))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--epochs", type=int, default=2100, help="Number of epochs to train.")
