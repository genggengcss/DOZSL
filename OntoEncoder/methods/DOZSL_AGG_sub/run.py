...
class Runner(object):
    ...
    def __init__(self, params):
        ...
        self.load_data()
        # add model
        model_name = '{}_{}'.format('DisenKGAT', self.p.score_func)
        if model_name.lower() == 'disenkgat_transe':
            # self.model = DisenKGAT_TransE(self.edge_index, self.edge_type, params=self.p)
            self.model = DisenKGAT_TransE(self.rel_edges, params=self.p)
        else:
