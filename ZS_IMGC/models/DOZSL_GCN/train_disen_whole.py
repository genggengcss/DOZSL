...
class Runner:
    ...
    def build_graph(self, features, threshold):
        ...
        adj = similarity_matrix.astype(float)
    ...
    def extract_embed_features(self, args):
        ...
        if args.DATASET == 'ImageNet/ImNet_A':
            embed_file = os.path.join(embed_path, 'KG_ImNet_A/DisenKAGT_TransE_mult_K4_D100_ImNet_A', '2200_2191_ent_embeddings.npy')
            entity_file = os.path.join(embed_path, 'KG_ImNet_A/ent2id.txt')

        # ImNet-O
        if args.DATASET == 'ImageNet/ImNet_O':
            embed_file = os.path.join(embed_path, 'KG_ImNet_O/DisenKAGT_TransE_mult_K5_D100_ImNet_O', '2000_1753_ent_embeddings.npy')
        ...
        for cls in self.all_classes:
            ...
            if cls in ent2id:
                if args.DATASET == 'ImageNet/ImNet_A':
                    vector = embeds[ent2id[cls]].reshape(400)
                if args.DATASET == 'ImageNet/ImNet_O':
                    vector = embeds[ent2id[cls]][[0, 1, 2, 3]].reshape(400)
                if args.DATASET == 'AwA2':
                    vector = embeds[ent2id[cls]].reshape(500)
...
if __name__ == '__main__':
    ...
    # parameters for GCN
    parser.add_argument('--input_dim', type=int, default=500, help='the dimension of node features')
